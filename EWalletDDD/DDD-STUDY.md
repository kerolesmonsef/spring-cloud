...
- **System (house) accounts**: `Account.openSystem(owner, currency, initialBalance)` (type SYSTEM, born with a genesis balance — no counterparty tx). `AccountApplicationService.ensureSystemAccount(currency, money)` is idempotent on `(type=SYSTEM, currency)` (`existsByTypeAndCurrency`). Seeder opens one per seeded currency, funded 1000, all under ONE shared system user (`findFirstByType(SYSTEM)` reuses the owner → no orphan users on re-run). Ledger port gained `existsByTypeAndCurrency` + `findFirstByType`. Deferred: genesis-balance-without-a-transaction is a seed shortcut; add a genesis entry if reconciliation ever needs it. Owner model = shared system user (alt: nullable `a_accounts.user_id`, blocked by ddl-auto not relaxing NOT NULL).
